...
export default function TerminalHome({ initialPoliticians }: TerminalHomeProps) {
  ...
  const [loading, setLoading] = useState(initialPoliticians.length === 0);
  const [error, setError] = useState<string | null>(null);
  const [feedItems, setFeedItems] = useState<FeedItem[]>(
    initialPoliticians.length > 0 ? generateFeedItems(initialPoliticians) : []
  );
  const [socialPosts, setSocialPosts] = useState<{ content: string; politician_name: string; posted_at: string }[]>([]);

  useEffect(() => {
    // If we already have server-provided data, just fetch social posts
    if (initialPoliticians.length > 0) {
      fetch('/api/social-posts?limit=5&order=desc')
    ...
    // Fallback: fetch everything client-side if no server data was provided
