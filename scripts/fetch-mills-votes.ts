#!/usr/bin/env npx tsx
/**
 * Fetch Rep. Cory Mills's (FL-07, bioguide M001216) full House voting record
 * (Jan 2023-present, 118th-119th Congresses) and filter to
 * Israel/Palestine/Iran/foreign-aid-related votes.
 *
 * Output: data-ingestion/mills-votes-historical.json
 *
 * NOTE: Read-only — does NOT touch Supabase or recompute scores.
 *
 * Usage:
 *   npx tsx scripts/fetch-mills-votes.ts
 *
 * IMPORTANT BIOGUIDE NOTE:
 * The task brief specified bioguide M001226, but per the canonical
 * unitedstates/congress-legislators dataset, M001226 is Rep. Rob Menendez (D-NJ-08).
 * Cory Mills's correct bioguide is M001216 (GovTrack id 456889, FEC H2FL07156).
 */
import * as fs from 'fs';
import * as path from 'path';

const GOVTRACK_BASE = 'https://www.govtrack.us/api/v2';
const BIOGUIDE_ID = 'M001216';
// Resolved via GET /api/v2/person/?q=Cory+Mills which returned profile link
// https://www.govtrack.us/congress/members/cory_mills/456889 (bioguide M001216).
// Cross-verified against unitedstates/congress-legislators legislators-current.yaml.
const MILLS_PERSON_ID = 456889;
const PAGE_SIZE = 200;
const RATE_LIMIT_MS = 250;
const OUTPUT_PATH = path.join(
  'data-ingestion',
  'mills-votes-historical.json',
);
...
const RELEVANCE_PATTERNS: Array<{ tag: Relevance; re: RegExp }> = [
  ...
  { tag: 'gaza', re: /\bgaza\b|\bhamas\b|\bhezbollah\b|\bhostages?\b/i },
  {
    tag: 'palestine',
    re: /palestin|west\s*bank|two[-\s]state|\bunrwa\b|palestinian authority|\bgolan\b|settlement|abraham\s*accords/i,
  },
  ...
  },
  {
    tag: 'iran',
    re:
      /\biran\b|jcpoa|nuclear agreement|ayatollah|tehran|revolutionary guard|\birgc\b|houthi/i,
  },
  {
    tag: 'foreign_aid',
    re:
 ...
 *   → $26B Israel-only pass-through component.
 * H.R. 8035 (118th) — Ukraine Security Supplemental Appropriations Act, 2024
 *   → Ukraine pass-through (part of same package as 8034).
 * H.R. 8036 (118th) — Indo-Pacific Security Supplemental Appropriations Act.
 */
const BILL_ALLOWLIST: Array<{
  ...
  {
    congress: 118,
    questionPattern: /\bH\.?\s*R\.?\s*8034\b/i,
    relevance: 'israel',
  },
  {
    congress: 118,
    questionPattern: /\bH\.?\s*R\.?\s*8035\b/i,
    relevance: 'foreign_aid',
  },
  {
    congress: 118,
    questionPattern: /\bH\.?\s*R\.?\s*8036\b/i,
    relevance: 'foreign_aid',
  },
];
...
function classifyCategory(
  ...
  // H.R. 8034 is Israel-only supplemental -> israel_aid_funding
  if (congress === 118 && /\bH\.?\s*R\.?\s*8034\b/i.test(question)) {
    return 'israel_aid_funding';
  }
  // H.R. 815 is the combined Israel/Ukraine/Taiwan supplemental
  if (congress === 118 && /\bH\.?\s*R\.?\s*815\b/i.test(question)) {
    return 'israel_aid_funding';
  }
  if (
  ...
  if (
    relevance === 'iran' ||
    /iran.*(sanction|nuclear|regime|missile)|jcpoa|\birgc\b|houthi/i.test(q)
  ) {
  ...
  if (
    relevance === 'palestine' ||
    relevance === 'gaza' ||
    /palestin|\bgaza\b|\bhamas\b|\bunrwa\b|two[-\s]state|west\s*bank|\bgolan\b|hostages?/i.test(q)
  ) {
...
/**
 * GovTrack caps offset at 1000 per query. Pull per half-year windows to stay
 * well under the cap.
 */
...
async function fetchAllMillsVotes(
  personId: number,
): Promise<GovTrackVoteVoter[]> {
  const all: GovTrackVoteVoter[] = [];
  // House FL-07: sworn in Jan 2023 (118th-119th Congress).
...
async function main(): Promise<void> {
  ...
  console.log('  Fetching Rep. Cory Mills voting record from GovTrack');
  console.log(`  Bioguide: ${BIOGUIDE_ID}  |  GovTrack: ${MILLS_PERSON_ID}`);
  console.log('='.repeat(60));

  await verifyPersonId(MILLS_PERSON_ID, BIOGUIDE_ID);

  console.log('\nFetching all vote records...');
  const allVotes = await fetchAllMillsVotes(MILLS_PERSON_ID);
  ...
  const israelAidVotes = relevant.filter(
    (r) => r.category === 'israel_aid_funding',
  );
  const iranVotes = relevant.filter(
    (r) => r.category === 'iran_sanctions_nuclear',
  );
  ...
  const nayOnAntiBds = antiBdsVotes.filter((r) => r.position === 'Nay');
  ...
  if (nayOnIsraelAid.length > 0) {
    console.log('  ---- Rare-bird NAY (Israel-aid) details: ----');
    for (const r of nayOnIsraelAid) {
      console.log(`    [${r.date}] ${r.bill}: ${r.title.substring(0, 100)}`);
    }
  }
  console.log(`  NAY votes on anti-BDS measures:  ${nayOnAntiBds.length}`);
  if (nayOnAntiBds.length > 0) {
    console.log('  ---- Rare-bird NAY (anti-BDS) details: ----');
    for (const r of nayOnAntiBds) {
      console.log(`    [${r.date}] ${r.bill}: ${r.title.substring(0, 100)}`);
    }
  }

  // Iran War Powers — keyword scan
  const warPowers = relevant.filter((r) =>
    /war\s*powers|withdraw.*forces|hostilities/i.test(
      `${r.title} ${r.description}`,
    ),
  );
  console.log(`\nWar Powers / withdrawal-style votes: ${warPowers.length}`);
  for (const r of warPowers) {
    console.log(
      `  [${r.date}] ${r.bill} (${r.position}, result=${r.result}): ${r.title.substring(0, 110)}`,
    );
  }

  // Explicit litmus checks for the FY24 supplemental package
  const supplementalBills: Array<{ label: string; pattern: RegExp }> = [
    { label: 'H.R. 815 combined', pattern: /\bH\.?\s*R\.?\s*815\b/i },
    { label: 'H.R. 8034 Israel-only', pattern: /\bH\.?\s*R\.?\s*8034\b/i },
    { label: 'H.R. 8035 Ukraine', pattern: /\bH\.?\s*R\.?\s*8035\b/i },
    { label: 'H.R. 8036 Indo-Pacific', pattern: /\bH\.?\s*R\.?\s*8036\b/i },
  ];
  for (const { label, pattern } of supplementalBills) {
    const matches = relevant.filter(
      (r) => r.congress === 118 && pattern.test(r.description),
    );
    if (matches.length > 0) {
      console.log(`\n${label} (118th) votes:`);
