#!/usr/bin/env npx tsx
/**
 * Fetch Rep. Troy Balderson's (OH-12, bioguide B001306) full Congressional
 * voting record in the House (Sep 2018-present, 115th-119th Congresses) and
 * filter to Israel/Palestine/Iran/foreign-aid-related votes.
 *
 * Output: data-ingestion/balderson-votes-historical.json
 *
 * NOTE: Read-only — does NOT touch Supabase or recompute scores.
 *
 * Usage:
 *   npx tsx scripts/fetch-balderson-votes.ts
 */
import * as fs from 'fs';
import * as path from 'path';

const GOVTRACK_BASE = 'https://www.govtrack.us/api/v2';
const BIOGUIDE_ID = 'B001306';
// GovTrack profile: https://www.govtrack.us/congress/members/troy_balderson/412747
const BALDERSON_PERSON_ID = 412747;
const PAGE_SIZE = 200;
const RATE_LIMIT_MS = 250;
const OUTPUT_PATH = path.join(
  'data-ingestion',
  'balderson-votes-historical.json',
);
...
async function fetchAllBaldersonVotes(personId: number): Promise<GovTrackVoteVoter[]> {
  const all: GovTrackVoteVoter[] = [];
  // House OH-12: Sep 2018-present (115th-119th Congress).
...
async function main(): Promise<void> {
  ...
  console.log('  Fetching Rep. Troy Balderson voting record from GovTrack');
  console.log(`  Bioguide: ${BIOGUIDE_ID}  |  GovTrack: ${BALDERSON_PERSON_ID}`);
  console.log('='.repeat(60));

  await verifyPersonId(BALDERSON_PERSON_ID, BIOGUIDE_ID);

  console.log('\nFetching all vote records...');
  const allVotes = await fetchAllBaldersonVotes(BALDERSON_PERSON_ID);
