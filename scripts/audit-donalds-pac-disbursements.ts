#!/usr/bin/env npx tsx
/**
 * CORRECTED pro-Israel PAC capture audit for Rep. Byron Donalds (R-FL-19).
 *
 * Forked from scripts/audit-rose-pac-disbursements.ts.
 *
 * Committee IDs:
 *   - Candidate:  H2FL14186 (BYRON DONALDS)
 *   - Principal cmte 2022-2026: C00733329 (BYRON DONALDS FOR CONGRESS)
 *   - Older cmte 2012-2016:     C00509877 (FRIENDS OF BYRON DONALDS)
 *
 * Correct method (avoids the broken Schedule A contributor_committee_id filter):
 *   GET /schedules/schedule_b/
 *       ?committee_id=<pro-israel PAC>
 *       &recipient_committee_id=<donalds cmte>
 *
 * Writes: data/donalds-pac-disbursement-audit.json
 */
...
const CANDIDATE_ID = 'H2FL14186';
const DONALDS_CMTES = [
  { id: 'C00733329', name: 'BYRON DONALDS FOR CONGRESS (2022-2026)' },
  { id: 'C00509877', name: 'FRIENDS OF BYRON DONALDS (2012-2016)' },
];

const ROOT = path.join(__dirname, '..');
const DATA_DIR = path.join(ROOT, 'data');
const INGEST_DIR = path.join(ROOT, 'data-ingestion');
const OUT_PATH = path.join(DATA_DIR, 'donalds-pac-disbursement-audit.json');
...
function loadRoseAipacEarmarkDonors(): Map<string, RoseAipacDonor> {
  if (!fs.existsSync(ROSE_PAC_AUDIT_PATH)) return new Map();
  const d = JSON.parse(fs.readFileSync(ROSE_PAC_AUDIT_PATH, 'utf8')) as {
    summary?: { top_earmark_donors?: RoseAipacDonor[] };
    by_pac?: Record<string, { earmarked_donors?: string[] }>;
  };
  const out = new Map<string, RoseAipacDonor>();
  for (const e of d.summary?.top_earmark_donors || []) {
    const k = (e.name || '').toUpperCase().replace(/\s+/g, ' ').trim();
    if (k) out.set(k, e);
  }
  for (const [pacName, pac] of Object.entries(d.by_pac || {})) {
    for (const dn of pac.earmarked_donors || []) {
      const k = (dn || '').toUpperCase().replace(/\s+/g, ' ').trim();
      if (!k) continue;
      if (!out.has(k)) out.set(k, { name: dn, total: 0, cycles: [], pacs: [pacName] });
    }
  }
  return out;
}
...
async function main(): Promise<void> {
  ...
  console.log('=== DONALDS PAC DISBURSEMENT AUDIT (CORRECTED METHOD) ===\n');
  console.log(`Candidate:  ${CANDIDATE_ID} (Byron Donalds, R-FL-19)`);
  for (const c of DONALDS_CMTES) console.log(`Donalds cmte:  ${c.id}  ${c.name}`);
  console.log('');
  ...
  const byPac: Record<
    string,
    {
      ...
      by_donalds_cmte: Record<string, number>;
      entries: ScheduleBEntry[];
  ...
  for (let i = 0; i < pacs.length; i++) {
    ...
    let combinedRows: Array<ScheduleBRow & { donalds_cmte: string }> = [];
    try {
      for (const dc of DONALDS_CMTES) {
        const rows = await pullScheduleBDisbursements(pac.id, dc.id);
        for (const r of rows) combinedRows.push({ ...r, donalds_cmte: dc.id });
      }
    } catch (e) {
    ...
    const entries: ScheduleBEntry[] = combinedRows.map(r => ({
      pac_id: pac.id,
      pac_name: pac.name,
      donalds_cmte: r.donalds_cmte,
    ...
    const byCmte: Record<string, number> = {};
    for (const e of entries) {
      const ck = String(e.cycle || 'unknown');
      byCycle[ck] = (byCycle[ck] || 0) + e.amount;
      byCmte[e.donalds_cmte] = (byCmte[e.donalds_cmte] || 0) + e.amount;
    }
    ...
    byPac[pac.name] = {
      ...
      by_cycle: byCycle,
      by_donalds_cmte: byCmte,
      entries,
      earmarked_donors: earmarkDonors,
    };
    allEntries.push(...entries);
    console.log(
      `  ${combinedRows.length} rows / $${total.toLocaleString(undefined, { maximumFractionDigits: 2 })}  cycles=${Object.keys(byCycle).sort().join(',')}  earmark_donors=${earmarkDonors.length}`,
    );
    await sleep(400);
  }
  ...
  const roseAipacIdx = loadRoseAipacEarmarkDonors();

  const crossBlackburn: Array<{
    donalds_donor_memo_form: string;
    blackburn_entry: BlackburnMatch;
    donalds_total: number;
    donalds_cycles: number[];
    donalds_pacs: string[];
  }> = [];
  const crossRoseAipac: Array<{
    donor: string;
    donalds_total: number;
    donalds_cycles: number[];
    donalds_pacs: string[];
    rose_entry: RoseAipacDonor;
  }> = [];
  for (const [key, d] of earmarkDonorAgg) {
    const normalized = normalizeDonorNameForMatch(d.name);
    const bhit = blackburnIdx.get(normalized);
    if (bhit) {
      crossBlackburn.push({
        donalds_donor_memo_form: d.name,
        blackburn_entry: bhit,
        donalds_total: d.total,
        donalds_cycles: [...d.cycles].sort(),
        donalds_pacs: [...d.pacs],
      });
    }
    const rhit = roseAipacIdx.get(key);
    if (rhit) {
      crossRoseAipac.push({
        donor: d.name,
        donalds_total: d.total,
  ...
  const cyclesOfInterest = ['2012', '2014', '2016', '2018', '2020', '2022', '2024', '2026'];
  ...
  const ieFor = ieEntries
    .filter(e => (e.support_oppose_indicator || '').toUpperCase() === 'S')
    .reduce((s, e) => s + (Number(e.expenditure_amount) || 0), 0);
  const ieAgainst = ieEntries
    .filter(e => (e.support_oppose_indicator || '').toUpperCase() === 'O')
    .reduce((s, e) => s + (Number(e.expenditure_amount) || 0), 0);
  summary.independent_expenditures_for_support = ieFor;
  summary.independent_expenditures_oppose = ieAgainst;
  summary.earmarked_donors_also_on_blackburn_tn_gov = crossBlackburn.map(c => ({
    ...
    to_donalds_earmark_total: c.donalds_total,
    via_pacs: c.donalds_pacs,
    cycles: c.donalds_cycles,
    foreign_money_flag: c.blackburn_entry.foreign_money_flag,
  }));
  summary.earmarked_donors_also_on_rose_aipac = crossRoseAipac.map(c => ({
    name: c.donor,
    to_donalds: c.donalds_total,
    to_rose_on_aipac_memo: c.rose_entry.total,
    cycles_donalds: c.donalds_cycles,
    pacs_donalds: c.donalds_pacs,
  }));
  ...
  const out = {
    ...
    principal_committee_ids: DONALDS_CMTES.map(c => c.id),
    method:
  ...
  console.log(`  shared with Blackburn TN gov: ${crossBlackburn.length}`);
  for (const c of crossBlackburn) {
    console.log(
      `    - ${c.blackburn_entry.name} (${c.blackburn_entry.state}) — Donalds $${c.donalds_total} via ${c.donalds_pacs.join('/')} | Blackburn $${c.blackburn_entry.to_blackburn}`,
    );
  }
  console.log(`  shared with Rose AIPAC earmark list: ${crossRoseAipac.length}`);
  for (const c of crossRoseAipac) {
    console.log(
      `    - ${c.donor} — Donalds $${c.donalds_total} | Rose (AIPAC memo) $${c.rose_entry.total}`,
    );
  }
  console.log(`  IE for-support: $${ieFor.toLocaleString()}`);
  console.log(`  IE oppose:      $${ieAgainst.toLocaleString()}`);
  console.log(`\nTop 5 earmark donors to Donalds (by total):`);
