#!/usr/bin/env npx tsx
/**
 * Pull Byron Donalds' federal House Schedule A individual contributions
 * across all cycles he has served (2022, 2024, 2026) plus older cmte (2012-2016).
 *
 * Committees:
 *   - C00733329 BYRON DONALDS FOR CONGRESS (2022-2026)
 *   - C00509877 FRIENDS OF BYRON DONALDS   (2012-2016)
 *
 * Saves aggregated-by-donor to:
 *   data/donalds-federal-individual-donors-aggregated.json
 *
...
const COMMITTEES: Array<{ id: string; cycles: number[] }> = [
  { id: 'C00733329', cycles: [2022, 2024, 2026] },
  { id: 'C00509877', cycles: [2012, 2014, 2016, 2018, 2020] },
];
const ROOT = path.join(__dirname, '..');
const OUT = path.join(ROOT, 'data', 'donalds-federal-individual-donors-aggregated.json');
...
function aggregate(allCycles: Record<string, Row[]>): {
  ...
  const donors = [...byKey.values()].sort(
    (a, b) => b.total_to_donalds - a.total_to_donalds,
  );
  return {
    donors,
    totals: {
...
async function main(): Promise<void> {
  console.log(`=== Donalds federal Schedule A (all cycles) ===`);
  const raw: Record<string, Row[]> = {};
  for (const { id, cycles } of COMMITTEES) {
    for (const c of cycles) {
      const rows = await pullCycle(id, c);
      const key = String(c);
      raw[key] = (raw[key] || []).concat(rows);
    }
  }
  ...
  for (const [c, v] of Object.entries(agg.totals.by_cycle).sort()) {
    console.log(`    cycle ${c}: ${v.rows} rows / $${v.total.toLocaleString()}`);
  }

  const payload = {
    committee_ids: COMMITTEES.map(c => c.id),
    candidate_id: 'H2FL14186',
    cycles: [...new Set(COMMITTEES.flatMap(c => c.cycles))].sort(),
