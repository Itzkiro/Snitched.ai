#!/usr/bin/env npx tsx
/**
 * Federal FEC audit of Rep. John W. Rose (R-TN-06).
 *
 * Scope:
 *  - Pulls Schedule A (itemized receipts) for his House principal committee
 *    C00652743 JOHN ROSE FOR TENNESSEE for the 2026 cycle (cycle-only scoring policy).
 *  - Pulls Schedule E (independent expenditures) FOR/AGAINST him (H8TN06094) for 2026.
 ...
 *
 * Writes:
 *   data-ingestion/rose-house-fec-2026-itemized.json   (raw Schedule A)
 *   data-ingestion/rose-house-fec-2026-IE.json          (raw Schedule E)
 *   data/rose-house-audit.json                          (aggregated audit)
 *
 * His TN Governor 2026 state-level fundraising (separate TN REF filing) will be
 * audited by another agent.
 */
...
const CANDIDATE_ID_HOUSE = 'H8TN06094';
const PRINCIPAL_COMMITTEE = 'C00652743';   // JOHN ROSE FOR TENNESSEE
const CYCLE = 2026;

const ROOT = path.join(__dirname, '..');
const DATA_DIR = path.join(ROOT, 'data');
const INGEST_DIR = path.join(ROOT, 'data-ingestion');
const OUT_ITEMIZED = path.join(INGEST_DIR, 'rose-house-fec-2026-itemized.json');
const OUT_IE = path.join(INGEST_DIR, 'rose-house-fec-2026-IE.json');
const OUT_AUDIT = path.join(DATA_DIR, 'rose-house-audit.json');
...
async function main(): Promise<void> {
  console.log('=== JOHN ROSE (R-TN-06) FEDERAL FEC AUDIT — CYCLE 2026 ===\n');
  console.log(`House candidate: ${CANDIDATE_ID_HOUSE}`);
  console.log(`Principal committee: ${PRINCIPAL_COMMITTEE} (JOHN ROSE FOR TENNESSEE)\n`);
  ...
  const individualRows = await pullAllScheduleA(PRINCIPAL_COMMITTEE, CYCLE, true, 15, 200);
  ...
  fs.writeFileSync(OUT_ITEMIZED, JSON.stringify({
    ...
    candidate_id: CANDIDATE_ID_HOUSE,
    cycle: CYCLE,
  ...
  // 3. Schedule E
  console.log(`\nPulling Schedule E (IE for/against H8TN06094)...`);
  const ieRows = await pullAllScheduleE(CANDIDATE_ID_HOUSE, CYCLE);
  ...
  // 5. Individual donor cross-ref against registry (HIGH CONFIDENCE ONLY)
  ...
  console.log(`  FOR Rose:     ${ieFor.length} rows / $${ieForSum.toLocaleString()}`);
  console.log(`  AGAINST Rose: ${ieAgainst.length} rows / $${ieAgainstSum.toLocaleString()}`);
  ...
  console.log(`\nIndividual-donor registry crossref:`);
  console.log(`  matched: ${matches.length} (${highConf.length} high / ${mediumConf.length} medium)`);
  console.log(`  high-conf gave Rose:  $${highConf.reduce((s,m)=>s+m.candidateTotal,0).toLocaleString()}`);
  ...
  const audit = {
    ...
    candidate: {
      ...
      leadership_pac_ids: [],
      notes: 'Running for TN Governor 2026 at state level (separate TN REF filing); House seat TN-06.',
    },
    cycle: CYCLE,
