#!/usr/bin/env npx tsx
/**
 * Fetch Rep. Byron Donalds's (FL-19, bioguide D000032) full House voting record
 * (2021-present, 117th-119th Congresses) and filter to
 * Israel/Palestine/Iran/foreign-aid-related votes.
 *
 * Output: data-ingestion/donalds-votes-historical.json
 *
 * NOTE: Read-only — does NOT touch Supabase or recompute scores.
 *
 * Usage:
 *   npx tsx scripts/fetch-donalds-votes.ts
 */
import * as fs from 'fs';
import * as path from 'path';

const GOVTRACK_BASE = 'https://www.govtrack.us/api/v2';
const BIOGUIDE_ID = 'D000032';
// Resolved via GET /person/?q=Byron+Donalds which returned profile link:
// https://www.govtrack.us/congress/members/byron_donalds/456808
const DONALDS_PERSON_ID = 456808;
const PAGE_SIZE = 200;
const RATE_LIMIT_MS = 250;
const OUTPUT_PATH = path.join(
  'data-ingestion',
  'donalds-votes-historical.json',
);
...
function classifyCategory(
  ...
  // H.R. 3237 is Iron Dome replenishment
  if (congress === 117 && /\bH\.?\s*R\.?\s*3237\b/i.test(question)) {
    return 'israel_aid_funding';
  }
  if (
...
async function fetchAllDonaldsVotes(
  personId: number,
): Promise<GovTrackVoteVoter[]> {
  const all: GovTrackVoteVoter[] = [];
  // House FL-19: sworn in Jan 2021 (117th-119th Congress).
...
async function main(): Promise<void> {
  ...
  console.log('  Fetching Rep. Byron Donalds voting record from GovTrack');
  console.log(`  Bioguide: ${BIOGUIDE_ID}  |  GovTrack: ${DONALDS_PERSON_ID}`);
  console.log('='.repeat(60));

  await verifyPersonId(DONALDS_PERSON_ID, BIOGUIDE_ID);

  console.log('\nFetching all vote records...');
  const allVotes = await fetchAllDonaldsVotes(DONALDS_PERSON_ID);
