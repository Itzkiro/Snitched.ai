...
function mapRowToPolitician(row: Record<string, unknown>): Politician {
...
export async function GET(
  ...
  try {
    ...
    if (client) {
      ...
      if (!error && data) {
        return NextResponse.json(mapRowToPolitician(data as Record<string, unknown>));
      }

      // If Supabase failed or returned nothing, fall through to static data
      if (error) {
        console.error('Supabase error fetching politician by id:', error);
      }
    }

    // Fall back to static JSON data
    const allPoliticians = getJsonPoliticians();
    const found = allPoliticians.find(p => p.id === id);
